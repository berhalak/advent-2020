...
fun <T> Iterable<T>.except(t: Iterable<T>): Iterable<T> = this.filter { !t.contains(it) }
...
class Map2d<T, K> : Iterable<Pair<T, Map1d<T, K>>> {
    ...
    operator fun set(i: T, v: Map1d<T, K>) {
        map[i] = v
    }
...
class Map3d<T, K> : Iterable<Pair<T, Map2d<T, K>>> {
    ...
    operator fun set(i: T, v: Map2d<T, K>) {
        map[i] = v
    }
...
class Map4d<T, K> : Iterable<Pair<T, Map3d<T, K>>> {
    ...
    operator fun set(i: T, v: Map3d<T, K>) {
        map[i] = v
    }
